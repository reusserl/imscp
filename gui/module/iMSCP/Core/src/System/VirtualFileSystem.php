...
use iMSCP\Config\Handler\File as ConfigHandlerFile;
use iMSCP\Crypt;
...
class VirtualFileSystem
{
	...
	protected function createFtpUser()
	{
		...
		if (!$stmt->rowCount()) {
			throw new \RuntimeException('Could not create FTP user: Cannot find gid/uid');
		}
		...
		$this->password = Crypt::randomStr(16);

		exec_query(
			'INSERT INTO ftp_users (userid, passwd, uid, gid, shell, homedir) VALUES (?, ?, ?, ?, ?, ?)', array(
			$this->user, Crypt::sha512($this->password), $row['admin_sys_uid'], $row['admin_sys_gid'], '/bin/sh',
