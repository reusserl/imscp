			...
			if (in_array($pluginStatus, array('touninstall', 'disabled'))) {
				if (!$this->pluginIsLocked($name)) {
					try {
						...
						$responses = $this->eventsManager->dispatch(Events::onBeforeUninstallPlugin, array(
							'pluginManager' => $this,
							'pluginName' => $name
						));
		...
		if ($this->pluginIsKnown($name)) {
			...
			if ($isSubaction || in_array($pluginStatus, array('toenable', 'disabled'))) {
				try {
					...
					$responses = $this->eventsManager->dispatch(Events::onBeforeEnablePlugin, array(
						'pluginManager' => $this,
						'pluginName' => $name
					));
		...
		if ($this->pluginIsKnown($name)) {
			...
			if ($isSubaction || in_array($pluginStatus, array('todisable', 'enabled'))) {
				try {
					...
					$responses = $this->eventsManager->dispatch(Events::onBeforeDisablePlugin, array(
						'pluginManager' => $this,
						'pluginName' => $name
					));
			...
			if (in_array($pluginStatus, array('toupdate', 'enabled'))) {
				try {
					...
					if ($ret == self::ACTION_SUCCESS) {
						...
						$responses = $this->eventsManager->dispatch(Events::onBeforeUpdatePlugin, array(
							'pluginManager' => $this,
							'pluginName' => $name,
		...
		if ($this->pluginIsKnown($name)) {
			...
			if (in_array($pluginStatus, array('todelete', 'uninstalled', 'disabled'))) {
				try {
					...
					$responses = $this->eventsManager->dispatch(Events::onBeforeDeletePlugin, array(
						'pluginManager' => $this,
						'pluginName' => $name
					));
					...
					if (!$responses->isStopped()) {
						...
						$this->eventsManager->dispatch(Events::onAfterDeletePlugin, array(
							'pluginManager' => $this,
							'pluginName' => $name
						));
		...
		if ($this->pluginIsEnabled($name) && !$this->pluginIsProtected($name)) {
			...
			if ($this->pluginUpdateProtectedFile()) {
				$this->eventsManager->dispatch(
					Events::onAfterProtectPlugin, array('pluginManager' => $this, 'pluginName' => $name)
				);
