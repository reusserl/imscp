...
class ApsInstanceController extends ApsAbstractController
{
	...
	protected function newAction()
	{
		...
		$instance = (new ApsInstance())
			->setPackage($package)
			->setSettings($this->getInstanceSettingService()->getSettingsFromMetadataFile($package));

		$serializationContext = (new SerializationContext())
			->setGroups(array('Default', 'New'));

		$this->getResponse()->setContent($this->getSerializer()->serialize($instance, 'json', $serializationContext));
	}
	...
	protected function createAction()
	{
		...
		//$instance = $this->getSerializer()->deserialize($json, self::INSTANCE_ENTITY_CLASS, 'json');
		/*
		...
		$this->getResponse()
			->setContent($this->getSerializer()->serialize(['errors' => $errMessages], 'json'))
			->setStatusCode(400);
		*/
	}
