...
function _client_generateDomainAliasRedirect($id, $status, $redirectUrl)
{
    if ($redirectUrl == 'no') {
        if ($status == 'ok') {
            return ['-', tohtml("alias_edit.php?id=$id"), tr('Edit')];
        }
        ...
        if ($status == 'ok') {
            return [$redirectUrl, tohtml("alias_edit.php?id=$id"), tr('Edit')];
        }
...
function _client_generateSubdomainRedirect($id, $status, $redirectUrl, $entityType)
{
    ...
    if ($status == 'ordered') {
        return [($redirectUrl == 'no') ? '-' : $redirectUrl, '#', tr('N/A')];
    }
    ...
    if ($stmt->rowCount()) {
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            list($actionEdit, $actionScriptEdit) = _client_generateCustomDnsRecordAction(
                'edit',
                ($row['owned_by'] === 'custom_dns_feature') ? $row['domain_dns_id'] : (
                ($row['owned_by'] === 'ext_mail_feature') ? $row['domain_id'] . ';' . ($row['alias_id'] ? 'alias' : 'normal') : null
                ),
    ...
    if ($status == 'ok') {
        if ($action == 'edit') {
            if ($ownedBy === 'custom_dns_feature') {
                return [tr('Edit'), tohtml("dns_edit.php?id=$id")];
            }

            if ($ownedBy === 'ext_mail_feature') {
                return [tr('Edit'), tohtml("mail_external_edit.php?item=" . urlencode($id))];
            }
        }

        if ($ownedBy === 'custom_dns_feature') {
            return [tr('Delete'), tohtml("dns_delete.php?id=$id")];
        }
    }
