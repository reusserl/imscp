...
function client_addSubdomain()
{
    ...
    if ($domainType == 'dmn') {
        ...
        if (in_array($subLabelAscii, ['cgi-bin', 'phptmp'])) {
    ...
    \iMSCP\Core\Application::getInstance()->getEventManager()->trigger(\iMSCP\Core\Events::onBeforeAddSubdomain, null, [
    ...
    exec_query($query, [$domainId, $subLabelAscii, $mountPoint, $forwardUrl, $forwardType, 'toadd']);

    \iMSCP\Core\Application::getInstance()->getEventManager()->trigger(\iMSCP\Core\Events::onAfterAddSubdomain, null, [
        'subdomainName' => $subdomainName,
        'subdomainType' => $domainType,
        'parentDomainId' => $domainId,
        'mountPoint' => $mountPoint,
