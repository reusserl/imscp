...
function ftp_generatePageData($mainDmn, $mainDmnId, $tpl)
{
    ...
    $cfg = \iMSCP\Core\Application::getInstance()->getConfig();
    $dmnList = ftp_getDomainList($mainDmn, $mainDmnId, isset($_POST['domain_type']) ? clean_input($_POST['domain_type']) : 'dmn');
    ...
    if (
        ...
        if ($ret) {
            ...
            \iMSCP\Core\Application::getInstance()->getEventManager()->trigger(
                \iMSCP\Core\Events::onBeforeAddFtp, null, [
                'ftpUserId' => $userid,
                'ftpPassword' => $passwd,
                'ftpUserUid' => $uid,
                'ftpUserGid' => $gid,
            ...
            try {
                ...
                if (!$stmt->rowCount()) {
                    ...
                    exec_query($query, [$groupName, $gid, $userid]);
                } else {
                    $row = $stmt->fetch(PDO::FETCH_ASSOC);
                    $query = "UPDATE `ftp_group` SET `members` = ? WHERE `groupname` = ?";
                    exec_query($query, ["{$row['members']},$userid", $groupName]);
                }
                ...
                if (!$quotaEntriesExist) {
                    ...
                    exec_query(
                        $query,
                        [$groupName, 'group', 'false', 'hard', $diskspaceLimit * 1024 * 1024, 0, 0, 0, 0, 0]);
                }
            ...
            if ($ret) {
                \iMSCP\Core\Application::getInstance()->getEventManager()->trigger(
                    \iMSCP\Core\Events::onAfterAddFtp, null, [
                    'ftpUserId' => $userid,
                    'ftpPassword' => $passwd,
                    'ftpUserUid' => $uid,
                    'ftpUserGid' => $gid,
