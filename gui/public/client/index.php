...
function client_generateSupportSystemNotices()
{
    ...
    if ($row['cnt']) {
        set_page_message(tr('You received %d new answer(s) to your support questions.', '<b>' . $row['cnt'] . '</b>'), 'info');
    }
}
...
function client_generateFeatureStatus($tpl)
{
    ...
    $tpl->assign([
        ...
        'APS_STANDARD' => customerHasFeature('aps_standard') ? $trYes : $trNo,
    ...
    if (customerHasFeature('backup')) {
        ...
        if (count($bkTranslation) > 0) {
            $tpl->assign('BACKUP_FEATURE_STATUS', '<span style="color: green;">' . sprintf(tr('Enabled for: %s'), implode(', ', $bkTranslation)) . '</span>');
        } else {
...
function client_makeTrafficUsage($domainId)
{
    ...
    $query = '
        ...
        AND
            `dtraff_time` < ?
    ';
    $stmt = exec_query($query, [$domainId, $fdofmnth, $ldofmnth]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
