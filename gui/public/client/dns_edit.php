...
function client_validate_SRV($record, &$errorString, &$dns, &$text)
{
    ...
    if (!is_number($record['dns_srv_port'])) {
    ...
    if (!is_number($record['dns_srv_ttl'])) {
    ...
    if (!is_number($record['dns_srv_prio'])) {
    ...
    if (!is_number($record['dns_srv_weight'])) {
    ...
    if (!in_array($record['srv_proto'], ['udp', 'tcp', 'tls'])) {
        return false;
    }
...
function client_generatePage($tpl, $dnsRecordId)
{
    ...
    if (!$dnsRecordId) { // Add DNS record
        ...
        $stmt = exec_query($query, [$dnsRecordId, $mainDomainId]);
    ...
    $dnsTypes = client_create_options(['A', 'AAAA', 'SRV', 'CNAME', 'TXT'], client_getPost('type', $data['domain_type']));
...
function client_saveDnsRecord($dnsRecordId)
{
    ...
    if ($dnsRecordClass != 'IN' || !in_array($dnsRecordType, ['A', 'AAAA', 'CNAME', 'SRV', 'TXT'])) {
        showBadRequestErrorPage();
    }
    ...
    if (in_array($dnsRecordType, ['A', 'AAAA', 'CNAME'])) {
