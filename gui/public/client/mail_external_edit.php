...
function _client_getVerifiedData($itemId, $itemType)
{
    ...
    return [
        'domain_id' => $domainId, 'item_id' => $itemId, 'item_name' => $row['name'], 'item_type' => $itemType,
        'external_mail_type' => $row['external_mail'], 'external_mail_dns_ids' => explode(',', $row['external_mail_dns_ids'])
    ];
}
...
function client_editExternalMailServerEntries($item)
{
    ...
    if (!empty($_POST)) {
        ...
        $data['to_update'] = (isset($_POST['to_update'])) ? $_POST['to_update'] : [];
        $data['to_delete'] = (isset($_POST['to_delete'])) ? $_POST['to_delete'] : [];
        ...
        if (!empty($verifiedData['external_mail_dns_ids'])) {
            ...
            if ($stmt->rowCount()) {
                ...
                exec_query($query, ['tochange', 'off', null, $verifiedData['item_id']]);
                send_request();
                set_page_message(
