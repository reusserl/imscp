        ...
        if (null !== $domainType) {
            ...
            if ($mailTypeNormal) {
                ...
                if (is_number($quota)) {
                    ...
                    if ($mainDmnProps['mail_quota'] != '0') {
                        ...
                        $quotaLimit = floor($mainDmnProps['mail_quota'] - ($row['quota']));
            ...
            try {
                ...
                \iMSCP\Core\Application::getInstance()->getEventManager()->trigger(
                    \iMSCP\Core\Events::onBeforeAddMail, null, [
                ...
                ';
                exec_query($query, [
                    $username, $password, $forwardList, $mainDmnProps['domain_id'], $mailType, $subId, 'toadd', '0',
                    NULL, $quota, $mailAddr
                ]);

                \iMSCP\Core\Application::getInstance()->getEventManager()->trigger(
                    \iMSCP\Core\Events::onAfterAddMail, null, [
                    'mailUsername' => $username,
                    'mailAddress' => $mailAddr,
...
function client_generatePage($tpl)
{
    ...
    if ($mainDmnProps['mail_quota'] != '0' && $quota >= $mainDmnProps['mail_quota']) {
        ...
        $mailType = (isset($_POST['account_type']) && in_array($_POST['account_type'], ['1', '2', '3'])) ? $_POST['account_type'] : '1';
        $tpl->assign([
