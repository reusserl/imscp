...
function uploadPlugin($pluginManager)
{
    ...
    if (isset($_FILES['plugin_archive'])) {
        $beforeMove = function ($tmpDirectory) {
            ...
            if (!checkMimeType($tmpFilePath, ['application/x-gzip', 'application/x-bzip2', 'application/zip'])) {
        ...
        $tmpArchPath = utils_uploadFile('plugin_archive', [$beforeMove, $tmpDirectory]);
        ...
        if ($tmpArchPath !== false) {
            ...
            try {
                ...
                set_page_message(tr('Unable to upload plugin archive: %s', $e->getMessage()), 'error');
...
function checkAction($pluginManager, $pluginName, $action)
{
    ...
    switch ($action) {
        ...
        case 'uninstall':
            ...
            break;
        case 'update':
            if (!in_array($pluginStatus, ['toupdate'])) {
            ...
            break;
        case 'change':
            if (!in_array($pluginStatus, ['tochange'])) {
        ...
        case 'delete':
            ...
            break;
        case 'protect':
            if (!in_array($pluginStatus, ['enabled'])) {
