        ...
        $stmt = exec_query(
            '
                ...
                WHERE
                    t1.domain_id = ?
            ',
            [$statusOk, $statusOk, $statusOk, $domainId]
        );
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
...
function admin_checkAndUpdateData($domainId)
{
    ...
    try {
        ...
        if ($data['reseller_aps_standard'] == 'yes') {
            $data['domain_aps_standard'] = (in_array($data['domain_aps_standard'], ['no', 'yes']))
        ...
        if (empty($errFieldsStack) && !isset($_SESSION['pageMessages'])) { // Update process begin here
            ...
            if ($data['domain_dns'] != $data['fallback_domain_dns'] && $data['domain_dns'] == 'no') {
                ...
                exec_query('UPDATE domain_dns SET domain_dns_status = ? WHERE domain_id = ? AND owned_by = ?',
                    ['todelete', $domainId, 'custom_dns_feature']
                );
            ...
            if ($data['domain_disk_limit'] != $data['fallback_domain_disk_limit']) {
                exec_query(
                    ...
                    ',
                    [$data['domain_name'], 'group', 'false', 'hard', $data['domain_disk_limit'] * 1048576, 0, 0, 0, 0, 0]
                );
            }
