...
function &reseller_getData($domainId, $forUpdate = false)
{
    ...
    if (null == $data) {
        ...
        if ($forUpdate) { // Post request
            ...
            $data['domain_aps_standard'] = isset($_POST['domain_aps_standard']) ? clean_input($_POST['domain_aps_standard']) : $data['domain_aps_standard'];
...
function _reseller_generateLimitsForm($tpl, &$data)
{
    ...
    $tplVars['TR_RESELLER_CONSUMPTION'] = isset($_SESSION['logged_from']) ? tr('Reseller consumption') : tr('Your consumption');
    ...
    $tplVars['TR_CUSTOMER_TRAFFIC_COMSUPTION'] = tohtml(bytesHuman($data['domainTraffic'], 'MiB')) . ' / ' . (($data['fallback_domain_traffic_limit'] != 0) ? tohtml(bytesHuman($data['fallback_domain_traffic_limit'] * 1048576)) : tr('Unlimited'));
    $tplVars['TR_RESELLER_TRAFFIC_COMSUPTION'] = tohtml(bytesHuman($data['current_traff_amnt'] * 1048576)) . ' / ' . (($data['max_traff_amnt'] != 0) ? tohtml(bytesHuman($data['max_traff_amnt'] * 1048576)) : tr('Unlimited'));

    // Disk space limit
    $tplVars['TR_DISK_LIMIT'] = tr('Disk space limit [MiB]') . '<br/><i>(0 ' . tr('unlimited') . ')</span>';
    $tplVars['DISK_LIMIT'] = tohtml($data['domain_disk_limit']);
    $tplVars['TR_CUSTOMER_DISKPACE_COMSUPTION'] = tohtml(bytesHuman($data['domain_disk_usage'], 'MiB')) . ' / ' . (($data['fallback_domain_disk_limit'] != 0) ? tohtml(bytesHuman($data['fallback_domain_disk_limit'] * 1048576)) : tr('Unlimited'));
    $tplVars['TR_RESELLER_DISKPACE_COMSUPTION'] = tohtml(bytesHuman($data['current_disk_amnt'] * 1048576)) . ' / ' . (($data['max_disk_amnt'] != 0) ? tohtml(bytesHuman($data['max_disk_amnt'] * 1048576)) : tr('Unlimited'));
...
function reseller_checkAndUpdateData($domainId)
{
    ...
    try {
        ...
        if ($data['reseller_aps_standard'] == 'yes') {
            $data['domain_aps_standard'] = (in_array($data['domain_aps_standard'], ['no', 'yes'])) ? $data['domain_aps_standard'] : $data['fallback_domain_aps_standard'];
        ...
        if (empty($errFieldsStack) && !isset($_SESSION['pageMessages'])) { // Update process begin here
            ...
            if ($data['domain_dns'] != $data['fallback_domain_dns'] && $data['domain_dns'] == 'no') {
                ...
                exec_query($query, [$domainId, 'custom_dns_feature']);
                $daemonRequest = true;
            }
            ...
            if ($data['domain_ip_id'] != $data['fallback_domain_ip_id']) {
                if ($data['domain_alias_limit'] != '-1') {
                    exec_query('UPDATE domain_aliasses SET alias_ip_id = ? WHERE domain_id = ?', [
